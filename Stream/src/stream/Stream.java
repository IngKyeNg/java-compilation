...
public class Stream {
  ...
  public static void main(String args[]) {
    ...
    System.out.println("\n-----print smallest item----");
    List<Integer> items = Arrays.asList(6, 2, 13, 4, 35, 1, 17, 8, 19);
    int min = items.get(0);
    for(int i=1; i<items.size();i++){
      if (min>items.get(i)) min = items.get(i);
    }
    ...
    IntSummaryStatistics stats = integers.stream().mapToInt(
    ...
    System.out.println("\n-----print smallest item----");
    min = items.stream().reduce((x,y)->x>y?y:x).get();
    System.out.println("Min item is " + min);
  ...
  private static int getCountEmptyStringUsingJava7(
      List<String> strings) {
    int count = 0;
  ...
  private static String getMergedStringUsingJava7(
      List<String> strings, String separator) {
  ...
  private static List<Integer> getSquares(
    ...
    for (Integer number : numbers) {
      Integer square
          = new Integer(number.intValue() * number.intValue());
